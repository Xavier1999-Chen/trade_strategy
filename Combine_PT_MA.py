...
class AlgoEvent:
    ...
    def check_trading_strategy(self, timestamp):
        """检查是否应该应用配对交易策略"""
        ...
        if self.market_regime == "random_walk" and len(self.symbols) >= 2:
            ...
            for symbol in self.symbols:
                self.check_ma_cross_signal(symbol, timestamp)
        else:
            self.pair_trading_active = False
            self.ma_cross_active = False
    ...
    def place_market_making_orders(self, symbol, md):
        ...
        # 检查市场状态并执行相应策略
        self.check_trading_strategy(md.timestamp)
        
        # 如果激活了配对交易或MA Cross策略，不执行做市策略
        if self.pair_trading_active or self.ma_cross_active:
            return
